...
export function MobileLobbyScreen() {
  ...
  return (
    <div className="mobile-billiards-bg h-full flex flex-col">
      {/* Background Effects */}
              ...
              {mpView === 'menu' && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onPointerDown={() => isGuest ? router.push(`/${locale}/login`) : void handleCreateRoom()}
                      className={`flex flex-col items-center gap-3 p-5 bg-blue-500/15 border border-blue-500/30 rounded-2xl transition-transform ${isGuest ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`}
                    >
                  ...
                  {mpError && (
                    <div className="mt-4 text-center">
                      ...
                      {isGuest && (
                        <button
                          onPointerDown={() => router.push(`/${locale}/login`)}
                          className="text-xs text-amber-400 font-semibold mt-1"
                        >
                          Criar conta →
                        </button>
                      )}
